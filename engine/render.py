...
Usage: render.py <input> <multi> [output] [--scale 1.0] [--fps TARGET] [--no-trt]
...
# (see engine/svp_backend.py) for every source, and an rgb24 pipe here would re-quantise those
...
if SVP_MODE and not NO_INTERP:
    ...
    # VapourSynth (see engine/svp_backend.py), interpolating to the target rate in-stream and
    ...
    _svp_hpy = svp_backend.write_host_script(
        inp=inp, w=W, h=H, nb=NB, fps_num=num, fps_den=den, vfr_dec=VFR_DEC,
        rate_num=SVP_RATE_NUM, rate_den=SVP_RATE_DEN, nvof=SVP_NVOF,
        svp_dir=SVP_DIR, ffmpeg=FFMPEG, resume_out_base=RESUME_OUT_BASE)
    _svp_host = subprocess.Popen(
        [sys.executable, _svp_hpy],
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=NO_WINDOW)
    svp_backend.start_stderr_pump(_svp_host)   # filters the API3 deprecation noise
